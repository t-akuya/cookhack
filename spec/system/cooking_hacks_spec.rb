...
RSpec.describe "時短裏技投稿", type: :system do
  ...
  context '時短裏技が投稿できるとき' do
    it 'ログインしたユーザーは正しい情報を入力すれば時短裏技投稿ができ、トップページに移動する' do
      ...
      }.to change { CookingHack.count }.by(1)
      # トップページへ遷移したことを確認する
...
RSpec.describe '時短裏技編集', type: :system do
  ...
  context '時短裏技の編集ができるとき' do
    it 'ログインしたユーザーは自分が投稿した時短裏技の編集ができる' do
      ...
      fill_in 'repertoire-name', with: "#{@cooking_hack1.title}" + "編集済"
      fill_in 'repertoire-comment', with: "#{@cooking_hack1.contents}" + "編集済"
      # 編集を投稿してもcooking_hackモデルのカウントは変わらないことを確認する
      expect{
        find('input[name="commit"]').click
      }.to change { CookingHack.count }.by(0)
      ...
      expect(page).to have_content("#{@cooking_hack1.title}"+"編集済")
    end
  end
